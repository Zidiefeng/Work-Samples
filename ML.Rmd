...
# boosting （not used）
# bm<- ada(formula=class ~ .,data=train_set_40,
#          iter=50,bag.frac=0.5,
#          control=rpart.control(maxdepth=20, cp=0.01,minsplit=20,xval=10))
# boosting_val_prob_st <- predict(bm, newdata = val_set_40, type = "prob")
# boosting_test_prob_st <- predict(bm, newdata = test_set_20, type = "prob")
...
########################## experiment on loss funciton done here (not used) ##########################



# customized logistic regression (preprocess data)
```{r}
set.seed(5072)
...
class_data <- data_used$class
X_lr <- data.frame(X,class=class_data)
head(X_lr)
str(X_lr)
write_feather(X_lr, 'data_lr.feather')
X_lr <- read_feather('data_lr.feather')
traindata <- X_lr[train,]
testdata <- X_lr[test,]
train.X <- X[train,]
train.y <- data_used$class[train]
test.X <- X[test,]
test.y <- data_used$class[test]
```


# customized logistic regression (default loss function )
```{r}
traindata$class <- as.factor(traindata$class)
testdata$class <- as.factor(testdata$class)
glm.fit <- glm(class~.,data=traindata, family=binomial)
glm.probs <- predict(glm.fit, type="response")  
pred_class_lr <- ifelse(glm.probs>0.5,1,0)
mean(pred_class_lr==traindata$class)
pred_test_prob <- predict(glm.fit,testdata,type = 'response')
pred_class_test <- ifelse(pred_test_prob>0.5,1,0)
mean(pred_class_test==testdata$class)
table(pred_class_test,testdata$class)
type.err(table(pred_class_test,testdata$class))

length(pred_class_test)
dim(test.X)
length(pred_test_prob)
grid <- 10 ^ seq(10, -2, length=100)
cv.out <- cv.glmnet(train.X, train.y, alpha=0,lambda=grid)
plot(cv.out)
lam <- cv.out$lambda.1se

fit <- cv.out$glmnet.fit
summary(fit)
ridge.pred <- predict(fit, 
                      s=lam,   
                      newx=test.X)
pred_class_glm <- ifelse(ridge.pred>0.5,1,0)
mean(pred_class_glm==test.y)
sum(ridge.pred>0.3 & ridge.pred<0.7)
table(pred_class_glm,test.y)
type.err(table(pred_class_glm,test.y))
#cv.out$lambda.min
```
...
dt <- mx.symbol.Variable("data")
label <- mx.symbol.Variable("label")
fc1 <- mx.symbol.FullyConnected(dt, num_hidden = 1, name = "fc1")
lreg <- mx.symbol.LogisticRegressionOutput(fc1, name = "lreg")

mx.set.seed(0)
model <- mx.model.FeedForward.create(lreg, X = train.X, y = train.y,
                                     ctx = mx.cpu(),
                                     num.round = 5,
...
#train acc
pred_train <- t(predict(model,train.X))
pred_train_class <- ifelse(pred_train>0.5,1,0)
mean(pred_train_class==train.y)
pred <- t(predict(model,test.X))
pred_plt <- data.frame(pred)
ggplot(pred_plt, aes(x = pred)) +
  geom_histogram(binwidth = 0.1, fill = "lightblue", colour = "black")
pred_class <- ifelse(pred>0.5,1,0)
mean(pred_class==test.y)
sum(pred_class==1)
(table_ori <- table(pred_class,test.y))
sum(pred>0.3 & pred<0.7)
roc.area(as.numeric(test.y), as.numeric(pred))
roc.plot(as.numeric(test.y), as.numeric(pred))
type.err(table_ori)
all.ori <- cbind(pred,pred_class,test.y)
all.ori<- data.frame(all.ori)
names(all.ori) <- c('prob','class','y')
type2.prob.ori <- all.ori$prob[all.ori$class==0 & all.ori$y==1]
ggplot(data.frame(type2.prob.ori), aes(x = type2.prob.ori)) +
  geom_histogram(binwidth = 0.1, fill = "lightblue", colour = "black")
```

# customized logistic regression (customized loss function )
```{r}
sigOut <- mx.symbol.Activation(fc1, act_type = "sigmoid", name = "sigOut")
y_hat <- mx.symbol.Reshape(sigOut,shape=0)
a_value <- 0.25
g_value <- 5
entropy1 <- label*mx.symbol.log(y_hat)
entropy2 <- (1-label)*mx.symbol.log(1-y_hat)


#penalize more
loss <- mx.symbol.MakeLoss(mx.symbol.negative(((a_value*(1-y_hat)^g_value))*entropy1 + (((1-a_value)*(y_hat)^g_value))*entropy2),name="loss")

#only penalize imbalanced
loss <- mx.symbol.MakeLoss(mx.symbol.negative(a_value*entropy1 + (1-a_value)*entropy2),name="loss")

#original loss function
loss <- mx.symbol.MakeLoss(mx.symbol.negative(entropy1 + entropy2),name="loss")
...
uinternals = internals(modelCustom$symbol)
...
#pred_custom <- predict(modelCustom,test.X)
pred_custom <- data.frame(predCustom)
ggplot(pred_custom, aes(x = predCustom)) +
  geom_histogram(binwidth = 0.1, fill = "lightblue", colour = "black")
pred_class_custom <- ifelse(pred_custom>0.5,1,0)
sum(pred_class_custom==1)
sum(test.y==1)
dim(pred_custom)
mean(pred_class_custom==test.y)
(table.cut<-table(pred_class_custom,test.y))
all.cust <- cbind(predCustom,pred_class_custom,test.y)
all.cust<- data.frame(all.cust)
names(all.cust) <- c('prob','class','y')
type2.prob.cust <- all.cust$prob[all.cust$class==0 & all.cust$y==1]
ggplot(data.frame(type2.prob.cust), aes(x = type2.prob.cust)) +
  geom_histogram(binwidth = 0.1, fill = "lightblue", colour = "black")



roc.area(as.numeric(test.y), as.numeric(pred_custom[[1]]))
roc.plot(as.numeric(test.y), as.numeric(pred_custom[[1]]))
type.err(table.cut)


# explore type2
all.cust <- data.frame(cbind(pred_custom,pred_class_custom,test.y))
names(all.cust) <- c('prob','class','y')
type2.prob <- all.cust$prob[all.cust$class==0 & all.cust$y==1]
ggplot(data.frame(type2.prob), aes(x = type2.prob)) +
  geom_histogram(binwidth = 0.1, fill = "lightblue", colour = "black")
```
